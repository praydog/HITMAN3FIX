...
std::unique_ptr<Patch> Patch::create(uintptr_t addr, const std::vector<uint8_t>& b, bool shouldEnable) {
...
bool Patch::patch(uintptr_t address, const vector<uint8_t>& bytes) {
...
Patch::Patch(uintptr_t addr, const std::vector<uint8_t>& b, bool shouldEnable /*= true*/)
...
bool Patch::enable() {
    ...
    if (m_original_bytes.empty()) {
        ...
        memcpy(m_original_bytes.data(), (void*)m_address, m_original_bytes.size());
    }
